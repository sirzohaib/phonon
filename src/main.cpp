...
void inputThreadFunc(ma_device &device) {
    while (running) {
        char c;
        std::cin >> c;
        if (c == 'p' || c == 'P') {
...
int main() {
    ...
    while (true) {
        ...
        httplib::Client cli("https://api.audius.co"); // the main api end point
        ...
        clearScreen(); // clear the screen obviously
        std::cout << "Loading Song... This may take a while...\n";
        ...
        std::cout << "Playing...Press 'p' to Pause/Resume, Press 'q' to Stop and when the song is finished.\n";
        std::thread inputThread(inputThreadFunc, std::ref(device));
        ...
        std::cout << "Would you like to play another song? : (y/n)";
        char choice;
        std::cin >> choice;
        ...
        if (choice == 'n') break; // exit outta here
